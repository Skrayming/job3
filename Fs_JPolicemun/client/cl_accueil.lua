...
local open = false 
local mainMenu = RageUI.CreateMenu('', 'Accueil') 
local subMenu1 = RageUI.CreateMenu('', 'Rendez Vous') 
...
function OpenMenuAccueilpolicem() 
    if open then 
		open = false
        ...
        RageUI.Button("Appel un Agent a l'accueil", nil, {RightLabel = "→"}, not codesCooldown5 , {
            onSelected = function()
            codesCooldown5 = true 
        TriggerServerEvent('Appel:policem')
        -- ESX.ShowNotification('~g~Votre message a bien été envoyé aux gendarme.')
        exports['okokNotify']:Alert("Police Municipale", "Votre message a bien été envoyé", 5000, 'success')
        ...
        RageUI.IsVisible(subMenu1, function()    
            ...
            RageUI.Button("Valider la Demande", "~o~Merci de respecter les Agents.", { Color = {BackgroundColor = { 76, 175, 80, 50}} }, true, {
                onSelected = function()
                    ...
                    RageUI.CloseAll()
                    TriggerServerEvent("Rdv:policemMotif", nomprenom, numero, heurerdv, rdvmotif)
                    ESX.ShowNotification("~b~policem~s~\nDemande de Rendez-vous envoyée")
                    nomprenom = nil
                    numero = nil
                    heurerdv = nil
                    rdvmotif = nil
...
local npc = {
	{hash="s_m_y_ranger_01", x = -2048.9848632813, y = -463.04476928711, z = 12.240758895874, a = 322.5850830078125},
}
